...
local num_epochs = std.parseInt(std.extVar('triplet_epochs'));
local batch_size = std.parseInt(std.extVar('triplet_bsz'));
...
local conj_loss_wt = std.parseJson(std.extVar('conj_loss_wt'));
local lr = std.parseJson(std.extVar('triplet_lr'));

local pretrained_path = std.extVar("triplet_pretrained_path");
